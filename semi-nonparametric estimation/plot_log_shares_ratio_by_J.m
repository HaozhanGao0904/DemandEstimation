...
for J = [5 25 50 100] 
    ...
    %share_tmp(share_tmp>.005) = NaN;
    share_in_true{1,i} = share_tmp;
    
    log_sh_ratio{1,i} = log(mean(sij,2)./mean(si0,2));
...
t = tiledlayout(2,2);
hist_J1 = nexttile;
histogram(log_sh_ratio{1,1},'Normalization','probability','BinWidth',.1);
title('J=5')

hist_J2 = nexttile;
histogram(log_sh_ratio{1,2},'Normalization','probability','BinWidth',.1);
title('J=25')

hist_J3 = nexttile;
histogram(log_sh_ratio{1,3},'Normalization','probability','BinWidth',.1);
title('J=50')

hist_J4 = nexttile;
histogram(log_sh_ratio{1,4},'Normalization','probability','BinWidth',.1);
title('J=100')

t.Padding = 'compact';
t.TileSpacing = 'compact';
t.XLabel.String = 'log(s_j/s_0)';
